...
public struct LightCompressor {
    public struct Video {
        public struct Configuration {
            ...
            public let videoBitrateInMbps: Float?
    ...
    public func compressVideo(videos: [Video],
        ...
        for (index, video) in videos.enumerated() {
            ...
            // set Max duration
            if configuration.maxDuration != nil {
                let maxDuration = CMTimeMakeWithSeconds(Float64(configuration.maxDuration!), preferredTimescale: 600)
                videoReader?.timeRange = CMTimeRange(start: .zero, duration: maxDuration)
                audioReader?.timeRange = CMTimeRange(start: .zero, duration: maxDuration)
            }
    ...
    private func getBitrate(bitrate: Float, quality: VideoQuality) -> Float {
        switch quality {
        case .very_high:
            return bitrate * 0.6
        case .high:
    ...
    private func generateWidthAndHeight(
        ...
        if width >= 2400 || height >= 2400 {
            newWidth = Int(width * 0.4 / 16) * 16
            newHeight = Int(height * 0.4 / 16 ) * 16
    ...
    private func getVideoWriterSettings(bitrate: Float, width: Int, height: Int) -> [String : AnyObject] {
